    ...
    var oTable = $("#fileData").dataTable({
      ...
      "columnDefs": [
        ...
        },
        {
          ...
          render: function(data, type, row) {
            idnya  = data.id_x;
            detail = "aksi('detail','"+idnya+"','','<?= $url_modal; ?>', 'md')";
            ...
            <?php if (check_permission('view', 'update', "master/toko")) { ?>
              if (data.status==1) {
              ...
              btn_aksi += '<a href="master/toko_harga/'+idnya+'" target="_blank" class="btn btn-icon rounded-circle glow btn-warning" data-toggle="tooltip" data-placement="top" title="harga"><i class="bx bx-dollar"></i></a>&nbsp;';

            <?php } ?>
            <?php if (check_permission('view', 'delete', "master/toko")) { ?>
              if (data.status==1) {
...
function btn_status(id='', stt='', msg='') {
  ...
  function(){
    // loading_show();
    hapus_data('master/simpan/toko_up_status/'+id, stt, '5', 1);
  });
}
</script>
