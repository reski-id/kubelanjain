...
function RefreshTable(status='') {
  ...
  dataSource = '<?= base_url("$url/list_data_plu/$tbl") ?>/'+status;
  if (oTable)
      oTable.fnDestroy();
      ...
      "columnDefs": [
        ...
        },
        {
          ...
          render: function(data, type, row) {
            ...
            <?php if (check_permission('page', 'update', "master/".uri(2))) { ?>
              if (data.status==1) {
                ...
                hapus     = "btn_status('"+idnya+"', 0, 'Ubah Status Item Kategori ke Tidak Aktif')";
                ...
                hapus     = "btn_status('"+idnya+"', 1, 'Ubah Status Item Kategori ke Aktif')";
...
function btn_status(id='', stt='', msg='') {
  ...
  function(){
    // loading_show();
    hapus_data('master/up_status_plu/<?= $tbl; ?>/'+stt, id, '5', 1);
  });
}
</script>
