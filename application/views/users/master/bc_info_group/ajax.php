...
function RefreshTable(stt='') {
  ...
  dataSource = '<?= base_url("$url/list_data/$tbl") ?>/'+stt;
  if (oTable)
      oTable.fnDestroy();
  ...
  if (fnServerObjectToArray) {
    var oTable = $("#fileData").dataTable({
      ...
      "columnDefs": [
        ...
        {
          "searchable": false, "orderable": false, className: "text-center align-top", "targets": 3,
          render: function(data, type, row) {
            idnya = data.id_x;
            ...
            <?php if(check_permission('view', 'update', "users/bc_info")){ ?>
            ...
            <?php } ?>
            <?php if(check_permission('view', 'delete', "users/bc_info")){ ?>
