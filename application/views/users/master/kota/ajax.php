...
function RefreshTable(status='', stt='') {
  stt = $('#v_sel_provinsi :selected').val();
  if (stt=='') {
    judul = '';
    $('#tabel_data').attr('hidden', true);
  }else {
    judul = ' - Provinsi <b>'+$('#v_sel_provinsi :selected').text()+'</b>';
    $('#tabel_data').removeAttr('hidden');
  }
  $('#judul_form_card').html('<?= $judul_web; ?>'+judul);
  ...
  dataSource = '<?= base_url("$url/list_data/$tbl") ?>/'+stt+'/'+status;
  if (oTable)
      oTable.fnDestroy();
