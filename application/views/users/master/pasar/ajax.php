  ...
  if (fnServerObjectToArray) {
    var oTable = $("#fileData").dataTable({
      ...
      "columnDefs": [
        ...
        {
          className: "text-left align-top", "targets": [2, 3, 5],
          render: function( data, type, row ){
            return data
          },
        },
        {
          className: "text-left align-top", "targets": [4],
          render: function( data, type, row ){
            return '<div class="get_data_kecnya" id="kec_v_'+row.id_pasar+'"></div>';
          },
        },
        {
          "searchable": false, "orderable": false, className: "text-center align-top", "targets": 6,
          render: function(data, type, row) {
            idnya  = data.id_x;
            ...
            <?php if (check_permission('view', 'update', "master/pasar")) { ?>
              if (data.status==1) {
                edit   = "aksi('edit','"+idnya+"','sync_form','<?= $url_modal; ?>', 'md')";
            ...
            <?php } ?>
            <?php if (check_permission('view', 'delete', "master/pasar")) { ?>
              if (data.status==1) {
...
function v_kec_multi(){
  $('.get_data_kecnya').each(function(i, obj) {
    var val = $(this).attr('id');
    var v_multi = $('#'+val);
    $.ajax({
      type: "POST",
      url : "<?php echo base_url('web/ajax_kec_multi'); ?>",
      data: "id="+val+'&status=1',
      dataType: "json",
      beforeSend: function(){
        v_multi.html('Memuat data kecamatan . . .');
      },
      success: function( data ) {
        ...
        if (data.nama=='') {
          ...
          $.each(data.nama, function(index, loaddata) {
            v_multi.append(loaddata.kec+', ');
          });
          v_multi.html(v_multi.html().slice(0, -2));
        }
      },
      error: function(){
        v_multi.html('<b>-</b>');
...
function btn_status(id='', stt='', msg='') {
  ...
  function(){
    // loading_show();
    hapus_data('master/simpan/pasar_up_status/'+id, stt, '5', 1);
  });
}
</script>
